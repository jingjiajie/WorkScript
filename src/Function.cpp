...
WorkScript::Function::Function(
	...
	:name(name), declaredReturnType(declaredReturnType), runtimeVarargs(isRuntimeVarargs), staticVarargs(isStaticVarargs), baseContext(baseContext)
{	
	...
	if (!matchedFunc) {
		...
		llvm::Function *func =
				llvm::Function::Create(funcType,
									   ...
									   Locale::fromWideChar(Encoding::ANSI, funcName),
		...
		for (size_t i = 0; i < llvmParamTypes.size(); ++i) {
			itArg->setName(Locale::fromWideChar(Encoding::ANSI, L"@" + to_wstring(i)));
			++itArg;
		}
